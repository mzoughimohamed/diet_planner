...
async def register(body: UserRegister, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    response.set_cookie("access_token", token, httponly=True, samesite="strict", max_age=settings.jwt_expire_minutes * 60)
    return user
...
async def update_me(
    ...
    if any(f in updates for f in ["weight_kg", "height_cm", "age", "gender", "activity_level", "goal"]):
